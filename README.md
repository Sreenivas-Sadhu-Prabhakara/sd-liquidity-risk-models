...
| **Asset Type** | Liquidity Risk Model |
| **Control Record** | Liquidity Risk Model Development Project |
...
| POST | `/v1/liquidity-risk-model-development-project/initiate` | Initiate |
| GET | `/v1/liquidity-risk-model-development-project` | Retrieve (list) |
| GET | `/v1/liquidity-risk-model-development-project/{crId}/retrieve` | Retrieve |
| PUT | `/v1/liquidity-risk-model-development-project/{crId}/update` | Update |
| PUT | `/v1/liquidity-risk-model-development-project/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/liquidity-risk-model-development-project/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-liquidity-risk-models:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-liquidity-risk-models ./helm -n bian-risk-compliance
```

Exposed through the platform Gateway at path prefix `/sd-liquidity-risk-models` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
